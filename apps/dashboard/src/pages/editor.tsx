...
export default function CapsuleEditor() {
  ...
  // Code generation hook for regeneration
  const { generateAndExecute, isCombinedProcessing, progress: regenProgress, currentStep: regenStep } = useCodeGeneration();
  ...
  const addTestCase = () => {
    ...
    setCapsuleData(prev => ({
      ...
      // Clear stale raw test cases so validation uses the edited ones
      _rawTestCases: undefined
    }));
  ...
  const regenerate = async () => {
    // Must be a generated capsule with language/difficulty info
    const language = (capsuleData as any).language;
    const difficulty = (capsuleData as any).difficulty;
    
    if (!language || !difficulty) {
      toast('warning', 'Cannot Regenerate', 'This capsule was not generated by AI. Use "Create Capsule" to generate a new one.');
      return;
    }

    // Use the problem statement or title as the prompt for regeneration
    const prompt = capsuleData.problemStatement || capsuleData.title;
    if (!prompt || prompt === 'Complete the function below.') {
      toast('warning', 'Missing Prompt', 'No problem statement available to regenerate from.');
      return;
    }

    setIsRegenerating(true);
    toast('info', 'Regenerating', 'Regenerating capsule with EdGE Forge (bypassing cache)...');
    ...
    try {
      const result = await generateAndExecute({
        ...
        skipCache: true, // Force fresh generation, bypass idempotency + semantic cache
      });

      if (result?.success && result.capsule) {
        const capsuleJson = result.capsule;
        const isSQL = (capsuleJson?.language || '').toLowerCase() === 'sql' || (capsuleJson?.type || '').toLowerCase() === 'sql';
        const database = capsuleJson?.content?.primary?.database || {};
        const code = capsuleJson?.content?.primary?.code?.wasmVersion || {};

        // Get test cases from the right location
        const rawTestCases = isSQL ? database.testCases : (code.testCases || capsuleJson.testCases);
        ...
        // Transform hints
        const transformedHints = (capsuleJson?.pedagogy?.hints?.sequence || capsuleJson?.hints || []).map((hint: any) => {
          return typeof hint === 'string' ? hint : hint.content || hint;
        });

        // Update capsule data with fresh generation
        setCapsuleData(prev => ({
          ...prev,
          title: capsuleJson.title || prev.title,
          problemStatement: capsuleJson?.content?.primary?.problemStatement || capsuleJson?.description || prev.problemStatement,
          hints: transformedHints.length > 0 ? transformedHints : prev.hints,
          solutionStub: isSQL ? (database.starterQuery || prev.solutionStub) : (code.starterCode || capsuleJson.starterCode || prev.solutionStub),
          referenceSolution: isSQL ? (database.solution || prev.referenceSolution) : (code.solution || capsuleJson.solution || prev.referenceSolution),
          ...
          expected_result: database.expected_result || (prev as any).expected_result || null,
          schema_definition: database.schema || (prev as any).schema_definition || '',
        }));

        toast('success', 'Regenerated!', `Capsule "${capsuleJson.title || capsuleData.title}" has been regenerated with fresh content.`);
      } else {
        const errorMsg = result?.error || 'Regeneration failed. Please try again.';
        toast('error', 'Regeneration Failed', errorMsg);
      }
    } catch (error) {
      console.error('Regeneration error:', error);
      toast('error', 'Regeneration Error', error instanceof Error ? error.message : 'An unexpected error occurred.');
    } finally {
  ...
  const handlePublishCapsule = async () => {
    try {
      ...
      if (result?.success) {
        ...
        showPublished(capsuleId);
        setPublishedCapsuleId(capsuleId);
  ...
  const handleValidateAndPublish = async () => {
    try {
      ...
      // Always use the current editor test cases (capsuleData.testCases)
      // These reflect any edits the user has made in the UI
      const validationTestCases = capsuleData.testCases || [];
      
      console.log('?? Test cases for validation:', {
        count: validationTestCases.length,
        sample: validationTestCases[0]
      });
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 border border-slate-700 rounded-xl shadow-2xl max-w-md w-full p-8 text-center">
            ...
            >
              <CheckIcon className="w-8 h-8" style={{ color: '#00ff87' }} />
            </div>
            <h2 className="text-2xl font-bold text-white mb-3">Capsule Published!</h2>
            ...
            <div className="space-y-3">
              <button
                ...
                style={{ background: 'linear-gradient(135deg, #00ff87, #00b894)' }}
                onMouseEnter={e => (e.currentTarget.style.background = 'linear-gradient(135deg, #00ef77, #00a884)')}
                onMouseLeave={e => (e.currentTarget.style.background = 'linear-gradient(135deg, #00ff87, #00b894)')}
              >
