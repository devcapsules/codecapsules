...
export function HeroSection() {
  ...
  return (
    <section className="relative overflow-hidden bg-[#04040a] min-h-screen flex items-center">
      {/* Orb glows */}
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-6 lg:py-14">
        <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          ...
          <div>
            ...
            {/* Headline */}
            <h1 className={`text-[2.1rem] sm:text-[2.6rem] lg:text-[3.2rem] font-black leading-[1.07] tracking-[-0.03em] text-white mb-5 transition-all duration-1000 delay-200 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            ...
            {/* Subheadline */}
            <p className={`text-base text-slate-400 mb-9 max-w-md leading-relaxed transition-all duration-1000 delay-400 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
            ...
            {/* CTA Buttons */}
            <div className={`flex flex-col sm:flex-row gap-3 mb-6 transition-all duration-1000 delay-600 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
              {!loading && user ? (
                <Link href="/dashboard"
          ...
          <div className={`transition-all duration-1000 delay-500 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            <div className="rounded-2xl p-4 sm:p-6 lg:p-8"
              ...
              <ProcessSteps />
              {/* Terminal status bar — signals live infra */}
              <div className="mt-4 pt-3 flex items-center gap-2" style={{ borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                <span className="w-1.5 h-1.5 rounded-full bg-[#00ff87] animate-pulse" style={{ boxShadow: '0 0 6px #00ff87' }} />
                <span className="font-mono text-[11px]" style={{ color: '#00ff87' }}>execution environment ready</span>
                <span className="ml-auto font-mono text-[11px] text-slate-600">v2.0.0</span>
              </div>
            </div>
          </div>
